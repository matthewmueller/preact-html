...
let Parser = typeof DOMParser === 'undefined'
 ...
 *  @param {String} markup
 */
module.exports = function parseMarkup (markup, type) {
  let mime = type === 'html' ? 'text/html' : 'application/xml'
  let parserError, wrappedMarkup, tag, doc
  ...
  try {
    doc = new Parser().parseFromString(wrappedMarkup, mime)
  } catch (err) {
    parserError = err
  }
